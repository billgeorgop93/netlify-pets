...
document.querySelector("#file-field").addEventListener("change", async function () {
  ...
  data.append("api_key", "987163916976712")
  data.append("signature", serverSignature)
  data.append("timestamp", serverTimestamp)

  // send to cloudinary
  const cloudinaryResponse = await axios.post("https://api.cloudinary.com/v1_1/dmnrhyggp/image/upload", data, {
  ...
  document.querySelector("#photo-preview").innerHTML = `<img src="https://res.cloudinary.com/dmnrhyggp/image/upload/w_190,h_190,c_fill/${cloudinaryResponse.data.public_id}.jpg" />`
